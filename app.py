...
app = Flask(__name__)
# Make the 'max' function available in all Jinja2 templates
    ...
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">
        ...
        <div class="hidden sm:flex items-center space-x-2">
          ...
          {% else %}
            <a href="{{ url_for('login') }}" class="px-4 py-2 text-sm font-medium text-gray-700 rounded-full hover:bg-gray-100 transition"> Log In </a>
            <a href="{{ url_for('register_parent') }}" class="px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-full hover:bg-blue-700 transition shadow-sm"> Register </a>
          {% endif %}
        </div>
        <div class="sm:hidden flex items-center"> <button id="mobile-menu-button" class="p-2 rounded-md text-gray-600 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition"> <svg class="h-6 w-6" stroke="currentColor" fill="none" viewBox="0 0 24 24" stroke-width="2"> <path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"/> </svg> </button> </div>
      </div>
    </div>
    <div id="mobileMenu" class="hidden sm:hidden bg-white shadow-lg">
      <div class="px-4 pt-4 pb-4 space-y-2">
        ...
        {% else %}
          ...
          <a href="{{ url_for('register_parent') }}" class="block px-3 py-2 rounded-md text-sm font-medium text-white bg-blue-600 hover:bg-blue-700"> Register </a>
        {% endif %}
      </div>
    </div>
  </nav>
    ...
    {% with messages = get_flashed_messages(with_categories=true) %}
      {% if messages %}
        {% for category, message in messages %}
          <div class="p-4 mb-6 text-sm rounded-lg shadow-md {{ 'bg-red-100 text-red-800 border border-red-200' if category == 'error' else 'bg-green-100 text-green-800 border border-green-200' }}" role="alert"> <span class="font-medium">{{ message }}</span> </div>
        {% endfor %}
      {% endif %}
    {% endwith %}
...
<div class="flex items-center justify-center min-h-[calc(100vh-250px)]">
  <div class="w-full max-w-md p-8 sm:p-12 space-y-6 bg-white rounded-2xl shadow-xl text-center">
    ...
    <a href="{{ url_for('register_child', invite_code=invite_code) }}"
       class="inline-block w-full py-3 px-4 font-semibold text-white bg-blue-600
...
def personal_dashboard():
    if current_user.role == 'parent':
        ...
        events = list(events_collection.find({ 'family_id': current_user.family_id, 'status': {'$in': ['completed', 'approved']} }).sort('due_date', DESCENDING).limit(20))
        reward_requests_cursor = rewards_collection.find({ 'family_id': current_user.family_id, 'status': 'requested' }).sort('_id', -1)
        reward_requests = []
        ...
        spend_tx = list(transactions_collection.find({ 'family_id': current_user.family_id }).sort('spent_at', DESCENDING))
        for t in spend_tx:
...
def family_dashboard():
    ...
    rec_cursor = events_collection.find({ 'family_id': fam_id, 'status': 'approved' }).sort('approved_at', DESCENDING).limit(5)
    recent_events = []
    for ev in rec_cursor:
        ...
        else: ev['approved_at_pretty'] = 'Recently'
        recent_events.append(ev)
    return render_full_template(FAMILY_DASHBOARD_TEMPLATE, stats=stats, family_members=family_members, recent_events=recent_events)
...
def register_parent():
    if request.method == 'POST':
        ...
        new_id = users_collection.insert_one({ 'email': email, 'username': username, 'password_hash': hashed_pw, 'role': 'parent', 'lifetime_points': 0, 'points': 0 }).inserted_id
...
def register_child(invite_code):
    ...
    if request.method == 'POST':
        ...
        hashed_pw = bcrypt.generate_password_hash(password).decode('utf-8')
        users_collection.insert_one({ 'username': username, 'password_hash': hashed_pw, 'role': 'child', 'family_id': invite_code, 'points': 0, 'lifetime_points': 0 })
...
def create_event():
    if current_user.role == 'parent':
        ...
        t = request.form['type']
        doc = { 'name': request.form['name'], 'description': request.form['description'], 'points': int(request.form['points']), 'type': t, 'family_id': current_user.family_id, 'status': 'assigned', 'created_at': datetime.utcnow(), 'assigned_to': request.form['assigned_to'], 'due_date': d }
        if t == 'habit':
            doc['streak'] = 0
...
def checkin_habit(event_id):
    if current_user.role == 'child':
        ...
        events_collection.update_one( {'_id': ObjectId(event_id)}, {'$set': { 'last_completed': datetime.utcnow(), 'streak': new_streak }} )
        users_collection.update_one( {'_id': ObjectId(current_user.id)}, {'$inc': { 'points': habit['points'], 'lifetime_points': habit['points'] }} )
...
def approve_event(event_id):
    if current_user.role == 'parent':
        ...
        if e and e.get('assigned_to'):
            users_collection.update_one( {'_id': ObjectId(e['assigned_to'])}, {'$inc': { 'points': e['points'], 'lifetime_points': e['points'] }} )
...
def request_reward():
    if current_user.role == 'child':
        ...
        if user.get('points', 0) < cost:
            ...
            return redirect(url_for('personal_dashboard'))
        users_collection.update_one( {'_id': ObjectId(current_user.id)}, {'$inc': {'points': -cost}} )
        reward_id = rewards_collection.insert_one({ 'name': request.form['name'], 'points_cost': cost, 'family_id': current_user.family_id, 'requested_by_id': current_user.id, 'status': 'requested', 'resolved_at': None }).inserted_id
        transactions_collection.insert_one({ 'reward_id': reward_id, 'family_id': current_user.family_id, 'child_id': current_user.id, 'child_username': current_user.username, 'reward_name': request.form['name'], 'points_spent': cost, 'status': 'pending', 'spent_at': datetime.utcnow(), 'resolved_at': None })
...
def api_events():
    ...
    if (member_id:=request.args.get('member')): query['assigned_to'] = member_id
